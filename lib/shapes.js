...
// Discord study group with H. Burke and A. Goodwin
...
class Shape {
  constructor(shapeColor) {
    ...
    // these might be children
  }

  setColor(shapeColor) {
    this.shapeColor.push(shapeColor)
  }
}
...
class Circle extends Shape {
  ...
  render() {
    return `<circle cx="150" cy="100" r="50" fill="${this.shapeColor}/></svg>`;
  }
}
...
class Triangle extends Shape {
  render() {
    return `
      <polygon points="100,20 10,180 190,180" style="fill:"${this.shapeColor}"/>
    </svg>`;
  }
}
...
class Square extends Shape {
  render() {
    return `
    <rect x = "50" y = "20" width = "150" height = "150" style = fill = "${this.shpaeColor}"/>
  </svg>`;
  }
}
